...
const evalBaseRoot = path.join(tmpRoot, 'english-real-eval')
...
const connectScript = path.join(projectRoot, 'scripts', 'english-connect.mjs')
...
function run(command, args, options = {}) {
  ...
  try {
    ...
    return { command: [command, ...args].join(' '), cwd, ms: Date.now() - started, output: output.trim() }
  } catch (error) {
...
function runMaybe(command, args, options = {}) {
  ...
  try {
    ...
    return { ok: true, command: [command, ...args].join(' '), cwd, ms: Date.now() - started, output: output.trim(), error: '' }
  } catch (error) {
    return {
      ok: false,
...
function runNpm(args, options = {}) {
  if (process.platform === 'win32') return run('cmd.exe', ['/d', '/s', '/c', ['npm', ...args].join(' ')], options)
  return run('npm', args, options)
}

function runNpmMaybe(args, options = {}) {
  if (process.platform === 'win32') return runMaybe('cmd.exe', ['/d', '/s', '/c', ['npm', ...args].join(' ')], options)
...
function setupProject() {
  ...
  writeText(path.join(evalRoot, 'package.json'), `${JSON.stringify({
    name: 'fame-english-real-eval-sample',
    private: true,
    type: 'module',
  ...
  writeText(path.join(evalRoot, 'README.md'), '# FAME English real eval sample\n')
}
...
function runRealEvaluation() {
  ...
  const jsonlRoute = runRoute(['--goal', 'edit JSONL knowledge base and sync indexes'])
  ...
  checks.push({
    ...
    evidence: { route_has_database_sync: jsonlRoute.route_ids.includes('route-agent-database-sync'), parsed_records: jsonlRecords.length },
  })

  const nodeRoute = runRoute(['--goal', 'run npm test after confirming package root', '--operation', 'build', '--tool-name', 'exec_command', '--working-directory', evalRoot, '--expected-output', 'npm test passes'])
  ...
  checks.push({
    ...
    evidence: { command: npmTest.command, output: npmTest.output, ms: npmTest.ms },
  })
  ...
  const gitRoute = runRoute(['--goal', 'prepare to edit files while protecting a dirty Git worktree', '--operation', 'write'])
  ...
  checks.push({
    ...
    evidence: { failure_signature_present: gitRoute.failure_signatures.includes('tool-failure-dirty-worktree-overwrite'), git_status: gitStatus.output },
  })
  ...
  const deleteBlocked = runRoute([
    '--operation', 'delete',
    '--goal', 'PowerShell delete directory but avoid accidental deletion',
    '--tool-name', 'exec_command',
    '--working-directory', '<project-root>',
    '--expected-output', 'list absolute path and wait for approval',
  ])
  const deleteScoped = runRoute([
    '--operation', 'delete',
    '--goal', 'PowerShell delete directory but avoid accidental deletion',
    '--tool-name', 'exec_command',
    '--working-directory', evalRoot,
    '--target', deleteTarget,
    '--allowed-path', evalRoot,
    '--expected-output', 'list absolute path and wait for approval',
  ])
  const resolvedDeleteTarget = run('powershell', ['-NoProfile', '-Command', `(Resolve-Path -LiteralPath ${psLiteral(deleteTarget)}).Path`], { cwd: evalRoot })
  ...
  const encodingRoute = runRoute([
    '--goal',
    'PowerShell Chinese output mojibake needs diagnosis',
    '--operation',
    'read',
    '--tool-name',
    'exec_command',
    ...
    evalRoot,
    '--expected-output',
    'UTF-8 encoding state and Chinese probe evidence',
  ])
  ...
  checks.push({
    ...
    ok: prompt.includes('FAME English open edition') && prompt.includes('ToolResultSummary') && prompt.includes('language-tree-hub'),
    scenario: 'scenario-external-agent-integration',
    evidence: { chars: prompt.length, has_semantic_anchor: prompt.includes('language-tree-hub'), has_summary_contract: prompt.includes('ToolResultSummary') },
  })
  ...
  checks.push({
    ...
    ok:
      ...
      connectJson.next_command.includes('route:english'),
    scenario: connectJson.route_decision?.scenario,
    evidence: { status: connectJson.status, agent: connectJson.agent?.id, blockers: connectJson.blockers, next_command: connectJson.next_command },
  })

  const wrongCwd = path.join(evalRoot, 'wrong-cwd')
  fs.mkdirSync(wrongCwd, { recursive: true })
  const firstFailureRoute = runRoute([
    '--goal', 'run npm test after confirming package root',
    '--operation', 'build',
    '--tool-name', 'exec_command',
    '--working-directory', wrongCwd,
    '--expected-output', 'npm test passes',
  ])
  ...
  const retryRoute = runRoute([
    '--goal', 'rerun npm test but avoid repeating the wrong working directory failure',
    '--operation', 'build',
    '--tool-name', 'exec_command',
    '--working-directory', evalRoot,
    '--expected-output', 'npm test passes',
    '--prior-failure-signature', failureSignature,
  ])
...
try {
  ...
  console.error(JSON.stringify({ ok: false, edition_id: 'english', kind: 'real_evaluation', error: error.message }, null, 2))
  process.exit(1)
}
